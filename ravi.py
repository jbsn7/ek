...
def check_intr():
    try:
        ...
        print("Abe lauranet chalu Kar. Internet bhosdi ke")
        sys.exit(2)       
...
def infinity ():
    ...
    s = st.replace("'","")
    target = s
    #print(f"{B}[{W}{i}{B}] {C} 1400000 Sms+Calls Sent ! : {G}{target}")
...
def main():
	...
	print(f"{W}\n-----------------------------------------------")
	print(f"{Y}⚡ This tool is only Gaand Marne ke liye !")
	print(f"{W}-----------------------------------------------")
	print(f"\n{G}Choose Any Option\n")
...
def res():
	...
	if r =='y':
		...
		print()
		print(f"Follow on Ig : {W}@krish_na_2568")
		exit()
print(Style.BRIGHT)
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 1000000000000, "call": 60000000, "mail": 1000000000}
        cc, target = "", ""
        if mode in ["sms", "call"]:
            cc, target = get_phone_info()
            if cc != "91":
                max_limit.update({"sms": 10000000})
        elif mode == "mail":
        ...
        while True:
            try:
                ...
                delay = float(input(
                    ...
                    .strip())
                # delay = 0
                max_thread_limit = (count//100) if (count//100) > 0 else 1
